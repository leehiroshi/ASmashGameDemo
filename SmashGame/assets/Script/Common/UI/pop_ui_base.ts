...
import {handler, genHandler} from "../util"
...
export class POP_UI_BASE extends cc.Component {
    ...
    */
    ...
    {
        console.log("show", this._ui_name, ...params);
        if(this.btn_close)
        {
        ...
        if(!overlay)
        {
            PoolMgr.getInstance().spawn(UI_CONFIG.overlay_bg, genHandler((bg_node:cc.Node):void=>{
                if(!this.is_show || this.node.getChildByName(pop_overlay_bg))
                {
                    PoolMgr.getInstance().despawn(UI_CONFIG.overlay_bg, bg_node);
                    return;
                }
    ...
    __hide__():void
    {
        console.log("hide", this._ui_name);
        if(this.btn_close)
        {
