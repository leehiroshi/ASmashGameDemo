...
export class pop_mgr
{
    ...
    show(path:string, transition?:UI_TRANSITION, ...params:any[]):void
    {
        ...
        ui.is_show = true;
        PoolMgr.getInstance().spawn(path, genHandler((node:cc.Node):void=>{
            if(!ui.is_show)
            {
                PoolMgr.getInstance().despawn(path, node);
                return;
            }
            ui.node = node;
            //应用过渡效果
            ...
            TimerMgr.getInst().once(0, utils.genHandler(() => {
    ...
    hide(path:string):void
    {
        ...
        if(ui.node)
        {
            PoolMgr.getInstance().despawn(path, ui.node);
            //调用hide
