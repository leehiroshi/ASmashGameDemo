...
export default class Logger {
    ...
    private curLevel: LogLevel = LogLevel.kDebug;    // 当前日志级别
    ...
    // 用于打印所有调试信息
    debug(...args: any[]) {
        this.log(LogLevel.kDebug, args);
    }

    // 原则上所有的代码分支都有一行这条日志
    trace(...args: any[]) {
        this.log(LogLevel.kTrace, args);
    }

    // 一般来说线上可能是这种日志级别，打印一些标志性信息
    info(...args: any[]) {
        this.log(LogLevel.kInfo, args);
    }

    // 一般来跟预期不一致的异常都应该打印这个结果
    warning(...args: any[]) {
        this.log(LogLevel.kWarning, args);
    }

    // 一般来说错误都应该打印这个级别的日志，注意这个级别的日志会把调用栈信息也打印出来
    fatal(...args: any[]) {
        this.log(LogLevel.kFatal, args);
    }
    ...
    private log(level: LogLevel, args: any[]): void {
        ...
        var logTitle = TimeUtil.getDateStr(Date.now()) + "[" + this.levelDesc[level] + "]";
